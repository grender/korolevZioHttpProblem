...
class MyZioHttpKorolev[R] {
  ...
  (implicit eff:  ZEffect): HttpApp[R, Throwable] = {
    ...
    def app(req: Request): ResponseZIO[R, Throwable] = req match {
      ...
      case req if matchWebSocket(req) => for {
        _<- ZIO.logInfo(f"routeWsRequest ${req.method} ${req.path}")
        r<- routeWsRequest(req, subPath(req.url.path, rootPath.segments.length), korolevServer)
      } yield r

      case req => for {
        _<- ZIO.logInfo(f"routeHttpRequest ${req.method} ${req.path}")
        r<-routeHttpRequest(rootPath, req, korolevServer)
      } yield r
    }
  ...
  private def handleHttpResponse(korolevServer: KorolevService[RIO[R, *]],
                                 korolevRequest: KorolevHttpRequest[RIO[R, *]]
                                    ...
                                    for {
    _<- ZIO.logInfo(f"handleHttpResponse ${korolevRequest.method.value} ${korolevRequest.pq.mkString}")
    r<-korolevServer.http(korolevRequest).flatMap {
